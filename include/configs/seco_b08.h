...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"mmcautodetect=no\0" \
	"mmcargs_emmc=setenv bootargs console=${console},${baudrate} " \
		"root=/dev/mmcblk0p2 rootwait rw rootfstype=${mmcrootfstype} ${m4mmcargs} consoleblank=0 ${b08_board_revision}\0" \
	"mmcargs_usd=setenv bootargs console=${console},${baudrate} " \
		"root=/dev/mmcblk1p2 rootwait rw rootfstype=${mmcrootfstype} ${m4mmcargs} consoleblank=0 ${b08_board_revision}\0" \
	"loadbootscript_usd=" \
		"fatload mmc 0:1 ${loadaddr} ${script};\0" \
	"loadbootscript_emmc=" \
		"fatload mmc 1:1 ${loadaddr} ${script};\0" \
	...
	"mmcboot=echo Booting from mmc ...; " \
		...
		"if test ${boot_fdt} = yes || test ${boot_fdt} = try; then " \
			"if run loadfdt; then " \
				"usdreset; bootz ${loadaddr} - ${fdt_addr}; " \
			"else " \
		...
		"else " \
			"bootz; " \
		"fi;\0" \
	"netargs=setenv bootargs console=${console},${baudrate} ${b08_board_revision}" \
		"root=/dev/nfs " \
...
#define CONFIG_BOOTCOMMAND \
	...
	"if mmc rescan; then " \
		"udooinit; " \
		"if run loadbootscript_usd; then " \
			"echo Booting from uSD; run mmcargs_usd;" \
			"setenv mmcdev 0; run bootscript;" \
		    "else " \
			"echo Booting from eMMC; run mmcargs_emmc;" \
			"setenv mmcdev 1; run loadbootscript_emmc; run bootscript;" \
		"fi; " \
		"udooinit; " \
		"if run loadimage; then " \
			"run mmcboot; " \
		"else echo Can t find valid boot method; " \
		"fi; " \
	"else echo Can t find valid boot method; fi"
#else
#define CONFIG_BOOTCOMMAND \
	"mmc dev ${mmcdev}; " \
	"if mmc rescan; then " \
